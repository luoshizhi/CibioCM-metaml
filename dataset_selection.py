...
def read_params(args):
    ...
    arg('out_f', metavar='OUTPUT_FILE', nargs='?',
        default=None, type=str, help="the output dataset file")

    arg('-z', '--feature_identifier', type=str,
        default='k__', help="the feature identifier\n")
    ...
    arg('-t', '--tout', action='store_true',
        help="transpose output dataset file\n")
...
def main(args):
    ...
    f = pd.read_csv(par['inp_f'], sep='\t', header=None,
                    index_col=0, dtype='unicode')
    f = f.T

    pf = pd.DataFrame()
    if par['select']:
        pf = pf.append(pd.DataFrame([s.split(':') for s in par['select'].split(
            ',')], index=['select']*(par['select'].count(',')+1)))
    if par['remove']:
        pf = pf.append(pd.DataFrame([s.split(':') for s in par['remove'].split(
            ',')], index=['remove']*(par['remove'].count(',')+1)))
    if par['include']:
        pf = pf.append(pd.DataFrame([s.split(':') for s in par['include'].split(
            ',')], index=['include']*(par['include'].count(',')+1)))
    if par['exclude']:
        pf = pf.append(pd.DataFrame([s.split(':') for s in par['exclude'].split(
            ',')], index=['exclude']*(par['exclude'].count(',')+1)))

    meta = [s for s in f.columns if sum(
    ...
    for i in range(len(pf)):
        ...
        if pf.index[i] == 'include':
            if pf.iloc[i, 0] != 'feature_level':
                ...
                feat = [s for s in feat if (
                    pf.iloc[i, 1] in s) | ('unclassified' in s)]
    ...
    f.drop(f.loc[:, feat].columns[f.loc[:, feat].max().astype(
        'float') == f.loc[:, feat].min().astype('float')], axis=1, inplace=True)

    if par['out_f']:
        if par['tout']:
            f.to_csv(par['out_f'], sep='\t', header=True,
                     index=False, line_terminator='\n')
        else:
            f.T.to_csv(par['out_f'], sep='\t', header=False,
